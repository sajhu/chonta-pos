...
async function main() {
  ...
  async function upsertInsumo(
    ...
    unit: "ML" | "UNIDAD",
    minThreshold: number,
    packaging?: { packageLabel: string; packageSize: number },
  ) {
    return prisma.insumo.upsert({
      where: { name },
      update: {},
      create: { name, unit, minThreshold, ...packaging },
    });
  }

  const insumoPoker = await upsertInsumo("Cerveza Poker", "UNIDAD", 12);
  const insumoAguila = await upsertInsumo("Cerveza Águila", "UNIDAD", 12);
  const insumoClubColombia = await upsertInsumo("Cerveza Club Colombia", "UNIDAD", 12);
  const insumoAgua = await upsertInsumo("Agua sin gas", "UNIDAD", 12);
  const insumoViche = await upsertInsumo("Viche", "ML", 1000, {
    packageLabel: "Botella 350ml",
    packageSize: 350,
  });
  const insumoAlmibarMora = await upsertInsumo("Almíbar mora", "ML", 500);
  const insumoAlmibarLulo = await upsertInsumo("Almíbar lulo-limonaria", "ML", 500);
  const insumoZumoLimon = await upsertInsumo("Zumo de limón", "ML", 500);
