...
export function RecentOrders({
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto space-y-2 py-2">
      {orders.length === 0 && <div className="text-slate-400 text-sm py-6 text-center">Sin ventas todavía</div>}
      {orders.slice(0, 30).map((o) => (
        <div key={o.id} className="text-sm border rounded-lg p-2">
          ...
          <div className="flex items-center justify-between mt-1">
            <span className={o.status === "ANULADA" ? "text-red-600 text-xs font-medium" : "text-slate-400 text-xs"}>
              {o.status === "ANULADA" ? `Anulada: ${o.cancelReason ?? ""}` : "Completada"}
            </span>
            <div className="flex gap-2">
              <button onClick={() => onReprint(o)} className="text-xs text-slate-600 underline">
                Reimprimir
              </button>
